...
const withInitPoolIdentifier = async (transaction, connection, wallet) => {
  ...
  transaction.add(
    (0, instruction_1.initPoolIdentifier)(connection, wallet, {
...
const withInitStakePool = async (transaction, connection, wallet, params) => {
  ...
  const identifierData = await (0, common_1.tryGetAccount)(() =>
    (0, accounts_2.getPoolIdentifier)(connection)
  );
  const identifier =
  ...
  transaction.add(
    (0, instruction_1.initStakePool)(connection, wallet, {
      identifierId: identifierId,
...
const withInitStakeEntry = async (transaction, connection, wallet, params) => {
  const [[stakeEntryId], originalMintMetadatId] = await Promise.all([
    (0, utils_2.findStakeEntryIdFromMint)(
      connection,
      wallet.publicKey,
      params.stakePoolId,
  ...
  transaction.add(
    await (0, instruction_1.initStakeEntry)(connection, wallet, {
...
const withAuthorizeStakeEntry = async (
  ...
  transaction.add(
    await (0, instruction_1.authorizeStakeEntry)(connection, wallet, {
      stakePoolId: params.stakePoolId,
      originalMintId: params.originalMintId,
    })
  );
...
const withDeauthorizeStakeEntry = async (
  ...
  transaction.add(
    await (0, instruction_1.deauthorizeStakeEntry)(connection, wallet, {
      stakePoolId: params.stakePoolId,
      originalMintId: params.originalMintId,
    })
  );
...
const withInitStakeMint = async (transaction, connection, wallet, params) => {
  ...
  transaction.add(
    (0, instruction_1.initStakeMint)(connection, wallet, {
      stakePoolId: params.stakePoolId,
...
const withClaimReceiptMint = async (
  ...
  transaction.add(
    await (0, instruction_1.claimReceiptMint)(connection, wallet, {
      stakeEntryId: params.stakeEntryId,
...
const withStake = async (transaction, connection, wallet, params) => {
  ...
  transaction.add(
    (0, instruction_1.stake)(connection, wallet, {
      stakeEntryId: stakeEntryId,
      stakePoolId: params.stakePoolId,
...
const withUnstake = async (transaction, connection, wallet, params) => {
  ...
  const stakePoolData = await (0, accounts_2.getStakePool)(
    connection,
    params.stakePoolId
  );
  if (
  ...
  transaction.add(
    (0, instruction_1.unstake)(connection, wallet, {
      stakePoolId: params.stakePoolId,
      stakeEntryId: stakeEntryId,
      originalMintId: params.originalMintId,
...
const withUpdateStakePool = (transaction, connection, wallet, params) => {
  transaction.add(
    (0, instruction_1.updateStakePool)(connection, wallet, {
      stakePoolId: params.stakePoolId,
      requiresCreators: params.requiresCreators || [],
      requiresCollections: params.requiresCollections || [],
...
const withUpdateTotalStakeSeconds = (
  transaction,
  connection,
  wallet,
  params
  ...
  transaction.add(
    (0, instruction_1.updateTotalStakeSeconds)(connection, wallet, {
...
const withReturnReceiptMint = async (
  ...
  const stakeEntryData = await (0, common_1.tryGetAccount)(() =>
    (0, accounts_2.getStakeEntry)(connection, params.stakeEntryId)
  );
  if (!stakeEntryData) {
  ...
  transaction.add(
    await (0, instruction_1.returnReceiptMint)(connection, wallet, {
...
const withCloseStakePool = (transaction, connection, wallet, params) => {
  transaction.add(
    (0, instruction_1.closeStakePool)(connection, wallet, {
      stakePoolId: params.stakePoolId,
      authority: wallet.publicKey,
    })
  );
...
const withCloseStakeEntry = (transaction, connection, wallet, params) => {
  transaction.add(
    (0, instruction_1.closeStakeEntry)(connection, wallet, {
      stakePoolId: params.stakePoolId,
      stakeEntryId: params.stakeEntryId,
      authority: wallet.publicKey,
    })
...
const withReassignStakeEntry = (transaction, connection, wallet, params) => {
  transaction.add(
    (0, instruction_1.reassignStakeEntry)(connection, wallet, {
      stakePoolId: params.stakePoolId,
      stakeEntryId: params.stakeEntryId,
...
const withInitStakeBooster = async (
  ...
  transaction.add(
    await (0, instruction_1.initStakeBooster)(connection, wallet, {
      ...params,
    })
  );
  return transaction;
...
const withUpdateStakeBooster = async (
  ...
  transaction.add(
    await (0, instruction_1.updateStakeBooster)(connection, wallet, {
      ...params,
    })
  );
  return transaction;
...
const withCloseStakeBooster = async (
  ...
  transaction.add(
    await (0, instruction_1.closeStakeBooster)(connection, wallet, {
      ...params,
    })
  );
  return transaction;
