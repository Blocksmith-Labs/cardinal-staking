...
export const withInitRewardEntry = async (transaction, connection, wallet, params) => {
    const [rewardEntryId] = await findRewardEntryId(params.rewardDistributorId, params.stakeEntryId);
...
export const withClaimRewards = async (transaction, connection, wallet, params) => {
    ...
    if (rewardDistributorData) {
        ...
        const [rewardEntryId] = await findRewardEntryId(rewardDistributorData.pubkey, params.stakeEntryId);
        ...
        transaction.add(await claimRewards(connection, wallet, {
            stakePoolId: params.stakePoolId,
            stakeEntryId: params.stakeEntryId,
...
export const withCloseRewardDistributor = async (transaction, connection, wallet, params) => {
    ...
    if (rewardDistributorData) {
        ...
        transaction.add(await closeRewardDistributor(connection, wallet, {
            stakePoolId: params.stakePoolId,
            rewardMintId: rewardDistributorData.parsed.rewardMint,
            remainingAccountsForKind,
        }));
...
export const withUpdateRewardEntry = async (transaction, connection, wallet, params) => {
    return transaction.add(await updateRewardEntry(connection, wallet, {
        stakePoolId: params.stakePoolId,
        stakeEntryId: params.stakeEntryId,
...
export const withCloseRewardEntry = async (transaction, connection, wallet, params) => {
    const [rewardDistributorId] = await findRewardDistributorId(params.stakePoolId);
    const [rewardEntryId] = await findRewardEntryId(rewardDistributorId, params.stakeEntryId);
