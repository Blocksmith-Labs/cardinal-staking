    ...
    mintInfos: [
      {
        ...
        fungible:
          (_a = params.fungible) !== null && _a !== void 0
            ? _a
            : params.amount && params.amount.gt(new BN(1)),
...
export const stakeAll = async (connection, wallet, params) => {
  ...
  if (mintInfosWithReceipts.length > 0) {
    for (let i = 0; i < mintInfosWithReceipts.length; i++) {
      ...
      const stakePoolData = decodeIdlAccount(
        stakePoolInfo,
        "stakePool",
        STAKE_POOL_IDL
      );
      ...
      transaction.add(
        await stakePoolProgram(connection, wallet)
          .methods.stakeProgrammable(
            amount !== null && amount !== void 0 ? amount : new BN(1)
          )
          .accountsStrict({
            stakeEntry: stakeEntryId,
            stakePool: params.stakePoolId,
...
export const unstakeAll = async (connection, wallet, params) => {
  ...
  for (const {
    ...
    if (
      ...
      const stakeEntryData = decodeIdlAccount(
        stakeEntryInfo,
        "stakeEntry",
        STAKE_POOL_IDL
      );
    ...
    if (
      ...
      const stakeEntry = decodeIdlAccount(
        stakeEntryInfo,
        "stakeEntry",
        STAKE_POOL_IDL
      );
